...
class CKFinder_Connector_CommandHandler_MoveFiles
	...
	function buildXml( oXML )
		Dim fileName, filePath, oDeleteFileNode, currentResourceTypeConfig
		...
		Dim clientPath, sServerDir, config, aclConfig, thumbnailsConfig
		...
		While (request.form("files[" & index & "][name]") <> "")
			...
			For i=1 To 1 ' VbScript lacks "continue" statement, let's fake it to avoid very complex nesting of IFs
				...
				Dim thumbsServerPath, thumbPath
                thumbsServerPath = oUFS.combinePaths(thumbnailsConfig.getDirectory(), config.getResourceTypeConfig(resourceTypeName).getName())
                thumbPath = oUFS.combinePaths(thumbsServerPath, path & name)
